...
contract OpenCompLike is ERC20 {
	using SafeMath for uint256;
	...
	constructor(address _admin) ERC20('OpenCompLike', 'TBIT') {
		admin = _admin;
	...
	function setPendingAdmin(address newPendingAdmin) external returns (bool) {
		if (msg.sender != admin) {
			revert('OpenCompLike:setPendingAdmin:illegal address');
		}
	...
	function acceptAdmin() external returns (bool) {
		if (msg.sender != pendingAdmin || msg.sender == address(0)) {
			revert('OpenCompLike:acceptAdmin:illegal address');
		}
	...
	function delegateBySig(
		...
		require(signatory != address(0), 'OpenCompLike::delegateBySig: invalid signature');
		require(nonce == nonces[signatory]++, 'OpenCompLike::delegateBySig: invalid nonce');
		require(block.timestamp <= expiry, 'OpenCompLike::delegateBySig: signature expired');
	...
	function getPriorVotes(address account, uint256 blockNumber) public view returns (uint256) {
		require(blockNumber < block.number, 'OpenCompLike::getPriorVotes: not yet determined');
	...
	function _writeCheckpoint(
		...
		uint256 blockNumber = safe32(block.number, 'OpenCompLike::_writeCheckpoint: block number exceeds 32 bits');
